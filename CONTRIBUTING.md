...
This also puts the Utile OS .1 releases on a very consistent **February** schedule, and the major Utile OS upgrades on the already discussed **August** schedule. However, for now, only August releases are expected.
...
* The `model` assertion is obtained through the API’s `/assertions/model/SERIES/TYPE/KEY`, or, in this case, `/assertions/model/16/generic/generic-classic`. `16` is currently a static value without much meaning. Note that an `Accept: ‘application/x.ubuntu.assertion’` header is required.
...
All three assertions are used to verify Canonical’s (or a brand’s) identity in relation to a pre-seed for Ubuntu Core (with that signed model assertion explicitly specifying which snaps and revisions are allowed). However, Desktop uses this generic model.
...
For the snap’s `.assert` files, each one actually has *three* assertions used by `snapd` to verify the whole package. They are, in order:
...
Now, I mentioned ‘helping the daemon’ earlier by doing some of the pre-seeding ourselves. How? The answer lies in yet another *undocumented* utility: [`snap-preseed`](https://github.com/canonical/snapd/blob/master/cmd/snapd/tool/snap-preseed/main.go). This program lives in `/usr/lib/snapd/`, and it runs part of the process the snap daemon runs when you first boot your PC, but it often gets stuck or finishes too late because the task is quite time-consuming. This program needs only one argument: a path to a [`chroot`](https://wiki.archlinux.org/title/Chroot) (a utility [used to change the apparent root filesystem for a process](https://wiki.debian.org/chroot)) into the target system, in this case the live environment before it is compressed into a SquashFS.
...
### The Build Process
Everything we’ve talked about so far culminates in this: how is an Ubuntu-based distribution such as Utile OS built? Why is doing some things better than others? This section covers that, but know that Utile OS’s build process (which we’re about to describe) isn’t expected to be perfect or foolproof, since it’s still very much a work in progress&hellip;
